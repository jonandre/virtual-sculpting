...
HRESULT KinectReader::CreateFirstConnected(StereoKinectHeadTracking* headTracking)
{
	...
	for (int i = 0; i < m_depthVector.size(); ++i) {
		...
		m_depthVector[i].Y = i/cDepthHeight;
	}
	// Done properly
...
void KinectReader::ProcessDepth()
{
    ...
    if (LockedRect.Pitch != 0)
    {
        ...
        while ( pBufferRun < pBufferEnd )
        {
			...
			m_depthVector[aux].Depth = (int)depth;
...
vector<DepthImagePoint>& KinectReader::GetDepthVector()
{
	return m_depthVector;
}

DepthImageFormat KinectReader::GetDepthImageFormat()
{
